class project_controller: public controller {
  ...
  public: 
    ...
    void create_project(name creator, string projectName, string contentType, uint32_t secondsToDeadline, uint64_t month) {
      ...
      the_transax_controller.send_self_deferred_action(creator, name("fail"), delay, std::make_tuple(creator, projectKey), projectKey);
      ...
      channels.modify(channelItr, get_self(), [&]( auto& row ) {
        row.mtotal_proj = row.mtotal_proj + 1;
      });
    }
    ...
    void fulfill_project(name creator, uint64_t projectKey) {
      ...
      the_transax_controller.cancel_deferred_transax(creator, name("fail"), projectKey); // cancel fail time action
